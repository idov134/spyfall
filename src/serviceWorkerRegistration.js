...
// register() is called in index.js so the whole game (bundle, styles,
// icons) is precached and can load and function fully offline after the
// first successful visit — important for players with no signal at all.

// This lets the app load faster on subsequent visits in production, and
// gives it offline capabilities. However, it also means that developers
// (and users) will only see deployed updates on subsequent visits to a
// page, after all the existing tabs open on the page have been closed,
